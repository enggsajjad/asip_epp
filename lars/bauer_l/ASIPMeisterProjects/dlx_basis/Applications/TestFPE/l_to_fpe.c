...
/*
 * signed long integral to fpe_t converter code
 *
 * Although unlikely, the long integral may have more bits than
 * fpe_t in its mantissa, so that rounding may be needed.
 */
void  l_to_fpe(fpe_p  op, long   il)
{
	register u_long		ul;
	...
	ul = (u_long)il;	/* ImplDep; jam value - should keep image */

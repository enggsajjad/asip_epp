 ...
 */
u_longer fpe_to_ull( register fpe_p ip )
{
	register u_longer	op;
	register int		n;
	register int		shift;
	...
	if ( ip->stat & (SIG|SIGNAN|NAN) ) {
		...
		n = _BPLL - 1;		/* == (almost) bits per long */
		do {
			if ( ( shift = n ) > _BPB ) {
				shift = _BPB;
			}
			n -= shift;
			op <<= shift;
			op |= (u_longer)frol_fpm( ip->mant, 0, shift );
		} while ( n > 0 );
		return op;
	}
	 ...
	 * Please note that this is (now) EXACTLY the to-signed conversion.
	 * Anyway, from now on, (-1.0:1.0) --> 0ull ( solving SPR 5067 )
	 */
	...
	op = ( u_longer )( ( 1ULL<<( _BPLL-2 ) )|( ( 1ULL<<( _BPLL-2 ) )-1 ) );
	if ( ip->stat & NEG ) {
		op = ~op;
	}
	return op;

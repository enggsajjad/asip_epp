...
#define PUTC(x) { value = (int) (x); PUTC_SINAS(value, putcgetcAddress); };
...
int getack (int address) {
  int value;
  int adr = address;
  int putcgetcAddress = LOAD_ADDRESS();

  PUTC_SINAS(0, putcgetcAddress);
  PUTC(++adr);
  PUTC_SINAS(1, putcgetcAddress);
  value = GETC_SINAS(putcgetcAddress);
...
int checkbuffer () {
  ...
  while (checksum == -1) {
    ...
    PUTC_SINAS(0, putcgetcAddress);
    PUTC(adr + 1);
    PUTC_SINAS(6, putcgetcAddress);

    value = GETC_SINAS(putcgetcAddress);  // ACK
    value = GETC_SINAS(putcgetcAddress);  // DC2
    checksum = value;
    length = GETC_SINAS(putcgetcAddress); // length
    checksum += length;
    bytes_ready = GETC_SINAS(putcgetcAddress);   // data
    checksum += bytes_ready;
    bytes_free = GETC_SINAS(putcgetcAddress);   // data
    checksum += bytes_free;
    bcc = GETC_SINAS(putcgetcAddress);    // bcc
...
int getbytes (char* dest, int bytes_to_read) {
  ...
  while (checksum == -1) {
    ...
    PUTC_SINAS(0, putcgetcAddress);
    PUTC(adr + 1);
    PUTC_SINAS(bytes_to_read + 4, putcgetcAddress);

    value = GETC_SINAS(putcgetcAddress);  // ACK
    // todo : check if ACK was ok
    value = GETC_SINAS(putcgetcAddress);  // DC1
    checksum = value;
...
int sendcommand (const char cmd0, const char cmd1, const int options[], const char text[], int intcount, int charcount, int address) {
  ...
  do {
    ...
    while (intcount_local-- != 0) {
      ...
      PUTC_SINAS(value, putcgetcAddress);
      checksum += value;
    }

...
int specialTable[] = {
  ...
  /* 24: */ OP_AVGU, OP_FEX,  OP_MAX,  OP_HT2X2CR,  OP_HT4X4IL,  OP_SATD4X4,   OP_DCT4X4,  OP_RES,
...
char *operationNames[] = {
  ...
  /* 55: */ "SUBUI", "XOR", "XORI", "NOP", "", "", "CMOV", "BEQ", "BNEQ",
  /* 64: */ "CADD", "BGTU", "BLEU", "BLTU", "BGEU", "MAC", "MAD", "SWAP", "XCHG",
  /* 73: */ "AVG", "AVGU", "FEX", "MAX", "HT2X2CR", "HT4X4IL", "SATD4X4", "DCT4X4"
};
/*  "HT2X2CR"   77, "HT4X4IL"   78, "SATD4X4"   79, "DCT4X4"       80*/
...
#define TEMP_STRING_SIZE 32
	...
	switch (wordPtr->opCode) {
	  ...
	  temp0   =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp1   =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp2   =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp3   =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp4   =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp5   =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp6   =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp7   =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp8   =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp9   =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp10  =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp11  =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp12  =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp13  =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp14  =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  if (ReadMem(machPtr, tmp, &value) == 0) goto stopSimulation;
	  temp15  =   ((value&0xFFFF0000)>>16);		  tmp+=(8*4);
	  ...
	  rd =   (((t0&0x0000FFFF)<<16)  | (t3&0x0000FFFF));
	  if (WriteMem(machPtr, tmp, 4, rd) == 0) goto stopSimulation;
	  tmp += 4;
	  rd  =   (((t1&0x0000FFFF)<<16)  | (t2&0x0000FFFF));
	  if (WriteMem(machPtr, tmp, 4, rd) == 0) goto stopSimulation;
	  tmp += 4;
	  rd  =   (((t4&0x0000FFFF)<<16)  | (t7&0x0000FFFF));
	  if (WriteMem(machPtr, tmp, 4, rd) == 0) goto stopSimulation;
	  tmp += 4;
	  rd  =   (((t5&0x0000FFFF)<<16)  | (t6&0x0000FFFF));
	  if (WriteMem(machPtr, tmp, 4, rd) == 0) goto stopSimulation;
	  tmp += 4;
	  rd =   (((t8&0x0000FFFF)<<16)  | (t11&0x0000FFFF));
	  if (WriteMem(machPtr, tmp, 4, rd) == 0) goto stopSimulation;
	  tmp += 4;
	  rd =   (((t9&0x0000FFFF)<<16)  | (t10&0x0000FFFF));
	  if (WriteMem(machPtr, tmp, 4, rd) == 0) goto stopSimulation;
	  tmp += 4;
	  rd =   (((t12&0x0000FFFF)<<16)	  | (t15&0x0000FFFF));
	  if (WriteMem(machPtr, tmp, 4, rd) == 0) goto stopSimulation;
	  tmp += 4;
	  rd =   (((t13&0x0000FFFF)<<16)	  | (t14&0x0000FFFF));
	  if (WriteMem(machPtr, tmp, 4, rd) == 0) goto stopSimulation;

	 // SHAF:  CYCLE INCREMENT
	machPtr->cycleCount+=HT4_Count;
	 printf("machPtr->cycleCount = %i\n",	machPtr->cycleCount);
	  }		
	  break;
	...
	case OP_SATD4X4:  /* Format: satd4x4 rd, rs1, rs2, rs3;   Semantic:  rd=Satd;  rs1 = &SB_start; rs2=Stride; rs3 = &Cand_start */
	{
		...
		/*	DIFFERENCE CALCULATION		*/
		{
			...
			diff0		=	(((((value1&0xFF000000)>>24) - ((value2 & 0xFF000000)>>24))<<16) | ((((value1&0x00FF0000)>>16) - ((value2 & 0x00FF0000)>>16))&0x0000FFFF));
			diff1		=	(((((value1&0x0000FF00)>>8) - ((value2 & 0x0000FF00)>>8))<<16) | (((value1&0x000000FF) - (value2 & 0x000000FF))&0x0000FFFF));
			address1	+=	(rs2*4); 	address2	+=	(4*4);
			
			if (ReadMem(machPtr, address1, &value1) == 0) goto stopSimulation;
			if (ReadMem(machPtr, address2, &value2) == 0) goto stopSimulation;
			diff2		=	(((((value1&0xFF000000)>>24) - ((value2 & 0xFF000000)>>24))<<16) | ((((value1&0x00FF0000)>>16) - ((value2 & 0x00FF0000)>>16))&0x0000FFFF));
			diff3		=	(((((value1&0x0000FF00)>>8) - ((value2 & 0x0000FF00)>>8))<<16) | (((value1&0x000000FF) - (value2 & 0x000000FF))&0x0000FFFF));
			address1 +=	(rs2*4);	address2 +=	(4*4);
			
			if (ReadMem(machPtr, address1, &value1) == 0) goto stopSimulation;
			if (ReadMem(machPtr, address2, &value2) == 0) goto stopSimulation;
			diff4		=	(((((value1&0xFF000000)>>24) - ((value2 & 0xFF000000)>>24))<<16) | ((((value1&0x00FF0000)>>16) - ((value2 & 0x00FF0000)>>16))&0x0000FFFF));
			diff5		=	(((((value1&0x0000FF00)>>8) - ((value2 & 0x0000FF00)>>8))<<16) | (((value1&0x000000FF) - (value2 & 0x000000FF))&0x0000FFFF));
			address1 +=	(rs2*4);	address2 +=	(4*4);
			
			if (ReadMem(machPtr, address1, &value1) == 0) goto stopSimulation;
			if (ReadMem(machPtr, address2, &value2) == 0) goto stopSimulation;
			diff6		=	(((((value1&0xFF000000)>>24) - ((value2 & 0xFF000000)>>24))<<16) | ((((value1&0x00FF0000)>>16) - ((value2 & 0x00FF0000)>>16))&0x0000FFFF));
			diff7		=	(((((value1&0x0000FF00)>>8) - ((value2 & 0x0000FF00)>>8))<<16) | (((value1&0x000000FF) - (value2 & 0x000000FF))&0x0000FFFF));
		}

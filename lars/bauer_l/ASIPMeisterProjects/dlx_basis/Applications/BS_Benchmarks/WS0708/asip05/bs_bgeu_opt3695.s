...
_bubbleSort:
	...
	; Save Registers 
	sw	 8(%r29), %r1		; r1 is used for comparing-results and forB temporary stuff.
	...
	nop
	slli	%r1, %r3, $2			; endIndex*4
	;nop
	addi	%r4, %r2, $0         		; i = startIndex;
	nop
	nop
...
_outterLoop:
	...
	bgeu	%r4, %r3,  _outterLoopEnd
	nop					; while (i < endIndex) {	
	
	;addi	%r7, %r5, $0			;   next_j = j;
	add	%r7, %r10, %r2			;   next_j = j;
	lw	%r8, 0(%r5)			;   value_j = *j;
	;addi	%r7, %r5, $0			;   next_j = j;	

	bgeu	%r5, %r6, _innerLoopEnd		;   while (j < innerLoopEnd) {
	nop
_innerLoop:
	addi 	%r7, %r7, $4			;     next_j++;
	lw	%r9, 4(%r7)			;     value_next_j = *next_j; ;r7'OLD
	nop
	addi	%r1, %r8, $0			;       tmp = value_j; //Early register overwriting
	addi	%r8, %r9, $0			;       value_j = value_next_j;
	bgeu	%r9, %r8, _endIf			
	nop
	;nop					;     if (value_next_j < value_j) {
	;addi	%r8, %r9, $0			;       value_j = value_next_j;
...
_endIf:
	bgeu	%r7, %r6, _innerLoopEnd		;   while (j < innerLoopEnd) {
	nop
	addi	%r5, %r7, $0			;     j = next_j;
	addi	%r8, %r9, $0			;     value_j = value_next_j;
	j	_innerLoop
	nop					;   }

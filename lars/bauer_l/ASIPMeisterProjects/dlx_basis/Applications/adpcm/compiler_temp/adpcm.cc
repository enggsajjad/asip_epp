...
#define signedUnsignedModification 0x8000
	...
	if (counter%10==9) separator=",\r\n";
	else separator=", ";
#else

	// Activate this part if you have stereo data
#if 0
	if (counter&1) writeToAudioOutL( (valpred+signedUnsignedModification) & 0xFFFF);
	else
#endif
	...
	//u_print("Required time for 1 audio-sample: ");
	//u_printInt(readClockCounter());
	//u_print(" cycles\r\n");
    }
...
int main() {
  ...
  t_print("ADPCM Decode: Start\r\n");

  adpcm_decoder(adpcmData, numberOfSamples);

  t_print("ADPCM Decode: Decoding Done.\r\n");
  return 0;
}

...
int main() 
{
	...
	int d		=	0;
			...
			forecast_SI_IPred_HDC16x16_73();
			forecast_SI_IPred_VDC16x16_73();
			forecast_SI_MC_Hz4_323();
			forecast_SI_MotionEstimation_39600();		//	396 x 100
			forecast_SI_LoopFilter_12672();			// 	396 x 32
	#else
	...
	for(i=150; i<150+numberOfImages; i++)
	{
		if(i != 150)
		{
			a += doMotionEstimation(i, numberOfMacroblocks);
			        ...
			        forecast_SI_MotionEstimation_0();
		...
		a += doLoopFilter(i, numberOfMacroblocks);	
                ...
                #ifndef START_UP_RECONFIGURATION
		        forecast_SI_MotionEstimation_39600();

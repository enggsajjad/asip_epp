...
int main() 
{
	...
	// Forecasts for the initial I-Frame
	forecast_SI_IPred_HDC16x16_396();
	forecast_SI_IPred_VDC16x16_396();

	// START THE IMAGE PROCESSING LOOP
	for(image=150; image<150+numberOfImages; image++)
	{

		// DUMMY LOOP TO REPRESENT MOTION ESTIMATION
		// It also provides some time for initial / modified reconfiguations
		for(i=0; i<10000; i++) 
		{
			retVal = retVal * i;
		}

		retVal += doEncodeMB(image, numberOfMacroblocks);

		// Forecasts for the subsequent frames
		forecast_SI_IPred_HDC16x16_46();
		forecast_SI_IPred_VDC16x16_46();
		forecast_SI_MC_Hz4_350();
	}

...
char inputString1[] = "Hi Welt Hi Welt";
char inputString2[1000];
//"Dies ist ein laengerer Test, der das Konzept pruefen soll...";
#define inputLength1 sizeof(inputString1)
#define inputLength2 sizeof(inputString2)

char encryptedString[max(inputLength1, inputLength2)];
char decryptedString[max(inputLength1, inputLength2)];

int dummy(char** argv) {
  Blowfish_Init(0, 0, 0);
  Blowfish_Encrypt(0, argv[2], 0);
  Blowfish_Decrypt(0, argv[1], 0);
  return (int)argv[1][0];
}
...
int main(int argc, char** argv) {
  ...
  inputString[inputLength] = '\0';  // ACHTUNG!!! Zugriff außerhalb des Arrays ??

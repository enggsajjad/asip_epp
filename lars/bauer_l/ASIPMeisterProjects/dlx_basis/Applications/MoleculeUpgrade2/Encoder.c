...
int doEncodeMB(const int imageNumber, const int numberOfMBs) {
  ...
  const int numberOfIMBs = imageNumber==150 ? 396 : I_MB[imageNumber];
  const int numberOfPMBs = imageNumber==150 ? 0 : (numberOfMBs - numberOfIMBs);
  ...
  for (currentMB=0; currentMB < numberOfMBs; currentMB++) {
    if ( numberOfIMBs>0 && (numberOfPMBs==0 || alreadyExecutedIMBs == 0 || ((alreadyExecutedPMBs<<7)/alreadyExecutedIMBs) > thresholdRatio ) ) {
      alreadyExecutedIMBs++;
#ifndef COSY
      printf("  Performing IMB    : executedIMBs: %i, executedPMBs: %i, threshold: %i\n", alreadyExecutedIMBs, alreadyExecutedPMBs, alreadyExecutedIMBs==0 ? 0 : ((alreadyExecutedPMBs<<7)/alreadyExecutedIMBs) );
#endif
      for (i=0; i<16; i++) {
	...
	IPred_VDC_16x16(refMB, 1, &DC_V);
      ...
      alreadyExecutedPMBs++;
#ifndef COSY
      printf("  Performing     PMB: executedIMBs: %i, executed PMBs: %i, threshold: %i\n", alreadyExecutedIMBs, alreadyExecutedPMBs, alreadyExecutedPMBs<<7/alreadyExecutedIMBs );
#endif
      for (i=0; i<16; i++) {
